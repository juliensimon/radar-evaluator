    ...
    def test_generate_radar_chart_empty_results(self):
        ...
        with patch(
            ...
            with patch("matplotlib.pyplot.savefig"):
                evaluator.generate_radar_chart([], "Model 1", "Model 2")
                # Should not raise an exception
                assert True

    def test_generate_radar_chart_single_industry(self):
        """Test radar chart generation with single industry results."""
        with patch(
            "builtins.open", mock_open(read_data=json.dumps(self.sample_config))
        ):
            evaluator = self.create_evaluator()
                ...
                with patch("pandas.DataFrame.to_csv") as mock_csv:
                    with patch("matplotlib.pyplot.savefig"):
                        with patch("matplotlib.pyplot.close"):
                            ...
                            # Note: savefig is called on the figure object, not plt directly
                            # So we check that the file operations happened
                            assert (
                                mock_file.call_count >= 2
                            )  # At least JSON and MD files
